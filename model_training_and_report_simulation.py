# -*- coding: utf-8 -*-
"""Football_DataSet_WC.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1yM7AFE6xI4ZbZToxeYzajBhT0qiMMPke
"""
...
# ------------------------------------------------------------
# SEED DO TORNEIO
# seed -> mantem a seed fixa em 42 
# ------------------------------------------------------------
#SEED = int(time.time()) % (2**32 - 1) -> gerar diferentes seeds 
SEED = 42
np.random.seed(SEED)
...
# RODAR FASE DE GRUPOS
import itertools

grupos = {}
for grupo in sorted(wc_teams['team_group'].unique()):
    selecoes = wc_teams[wc_teams['team_group'] == grupo]['national_team_id'].tolist()
    grupos[grupo] = selecoes

tabela = {tid: {'pts': 0, 'sg': 0, 'gp': 0, 'group': g}
          for g, ids in grupos.items() for tid in ids}
...
for grupo, times in grupos.items():
    print(f"\nGrupo {grupo}")
    for home_id, away_id in itertools.combinations(times, 2):
        resultado, g_home, g_away, _ = simular_partida(
            home_id, away_id, base_selecoes, modelo_final, scaler
        )
        if resultado == 0:
            tabela[home_id]['pts'] += 3
        elif resultado == 1:
            tabela[home_id]['pts'] += 1
            tabela[away_id]['pts'] += 1
        else:
            tabela[away_id]['pts'] += 3

        tabela[home_id]['sg'] += g_home - g_away
        tabela[away_id]['sg'] += g_away - g_home
        tabela[home_id]['gp'] += g_home
        tabela[away_id]['gp'] += g_away
...
df_tabela = pd.DataFrame([
    {
        'national_team_id': tid,
        'group'           : v['group'],
        'pts'             : v['pts'],
        'sg'              : v['sg'],
        'gp'              : v['gp'],
    }
    for tid, v in tabela.items()
])

df_tabela = df_tabela.merge(
    base_selecoes[['national_team_id', 'valor_total', 'national_team_name', 'fifa_ranking']],
    on='national_team_id', how='left'
)

# Desempate: pts → sg → gp → fifa_ranking (menor = melhor)
df_tabela = df_tabela.sort_values(
    ['group', 'pts', 'sg', 'gp', 'fifa_ranking'],
    ascending=[True, False, False, False, True]
)

df_tabela['pos'] = df_tabela.groupby('group').cumcount() + 1

classificados_diretos = df_tabela[df_tabela['pos'] <= 2].copy()

terceiros = df_tabela[df_tabela['pos'] == 3].sort_values(
    ['pts', 'sg', 'gp', 'fifa_ranking'],
    ascending=[False, False, False, True]
).head(8)

classificados = pd.concat([classificados_diretos, terceiros], ignore_index=True)

print("=" * 60)
print("CLASSIFICAÇÃO FINAL — FASE DE GRUPOS")
print("=" * 60)

for grupo in sorted(df_tabela['group'].unique()):
    print(f"\nGrupo {grupo}")
    grupo_df = df_tabela[df_tabela['group'] == grupo].sort_values('pos')
    for _, row in grupo_df.iterrows():
        classificado = "✅" if row['national_team_id'] in classificados['national_team_id'].values else "❌"
        print(f"  {row['pos']}º {row['national_team_name']:25s} "
              f"{row['pts']}pts  SG:{row['sg']:+d}  "
              f"GP:{row['gp']}  FIFA:{int(row['fifa_ranking'])}  {classificado}")

print(f"\n🔎 8 melhores terceiros:")
for _, row in terceiros.iterrows():
    print(f"  3º Grupo {row['group']} — {row['national_team_name']} "
          f"({row['pts']}pts  SG:{row['sg']:+d}  FIFA:{int(row['fifa_ranking'])})")

pos_por_grupo = {}
for grupo in sorted(df_tabela['group'].unique()):
    grupo_df = df_tabela[df_tabela['group'] == grupo].sort_values('pos')
    pos_por_grupo[grupo] = {
        int(row['pos']): row['national_team_id']
        for _, row in grupo_df.iterrows()
    }

terceiros_por_grupo = {
    row['group']: row['national_team_id']
    for _, row in terceiros.iterrows()
}

print(f"\n✅ Classificados prontos para o mata-mata")
...
# Removendo espaços em branco dos nomes dos grupos para evitar KeyError
pos_por_grupo_limpo = {k.strip(): v for k, v in pos_por_grupo.items()}
pool_terceiros_limpo = {k.strip(): v for k, v in terceiros_por_grupo.items()}

def sortear_terceiro(grupo_adversario, terceiros_disponiveis):
    candidatos = [
        tid for grupo, tid in terceiros_disponiveis.items()
        if grupo != grupo_adversario
    ]
    if not candidatos:
        candidatos = list(terceiros_disponiveis.values())

    escolhido       = np.random.choice(candidatos)
    grupo_escolhido = [g for g, tid in terceiros_disponiveis.items()
                       if tid == escolhido][0]
    del terceiros_disponiveis[grupo_escolhido]
    return escolhido
...
confrontos_fixos = [
    ('A', 2, 'B', 2), ('F', 1, 'C', 2), ('C', 1, 'F', 2), ('E', 2, 'I', 2),
    ('H', 1, 'J', 2), ('J', 1, 'H', 2), ('K', 2, 'L', 2), ('D', 2, 'G', 2),
]

confrontos_com_terceiro_A = [('E', 1), ('I', 1), ('A', 1), ('L', 1)]
confrontos_com_terceiro_B = [('D', 1), ('G', 1), ('B', 1), ('K', 1)]

chave_16avos = []

for g_home, pos_home, g_away, pos_away in confrontos_fixos:
    chave_16avos.append((pos_por_grupo_limpo[g_home][pos_home], pos_por_grupo_limpo[g_away][pos_away]))

for grupo, pos in confrontos_com_terceiro_A:
    home_id  = pos_por_grupo_limpo[grupo][pos]
    terceiro = sortear_terceiro(grupo, pool_terceiros_limpo)
    chave_16avos.append((home_id, terceiro))

for grupo, pos in confrontos_com_terceiro_B:
    home_id  = pos_por_grupo_limpo[grupo][pos]
    terceiro = sortear_terceiro(grupo, pool_terceiros_limpo)
    chave_16avos.append((home_id, terceiro))

print("=" * 60)
print("  DEZESSEIS-AVOS DE FINAL (32 → 16)")
print("=" * 60)
times_16 = [simular_confronto_mata_mata(h, a, base_selecoes, modelo_final, scaler) for h, a in chave_16avos]

print(f"\n{'='*60}")
print("  OITAVAS DE FINAL (16 → 8)")
print("=" * 60)
times_8 = [simular_confronto_mata_mata(times_16[i], times_16[i+1], base_selecoes, modelo_final, scaler) for i in range(0, 16, 2)]

print(f"\n{'='*60}")
print("  QUARTAS DE FINAL (8 → 4)")
print("=" * 60)
times_4 = [simular_confronto_mata_mata(times_8[i], times_8[i+1], base_selecoes, modelo_final, scaler) for i in range(0, 8, 2)]

print(f"\n{'='*60}")
print("  SEMIFINAL (4 → 2)")
print("=" * 60)
times_2 = [simular_confronto_mata_mata(times_4[i], times_4[i+1], base_selecoes, modelo_final, scaler) for i in range(0, 4, 2)]

print(f"\n{'='*60}")
print("  🏆 GRANDE FINAL")
print("=" * 60)
campeao_id = simular_confronto_mata_mata(times_2[0], times_2[1], base_selecoes, modelo_final, scaler)
vice_id = times_2[1] if campeao_id == times_2[0] else times_2[0]
